 ...
 *             if the result is inexact but the rounding mode is {@code UNNECESSARY}.
 * @throws ArithmeticException
 *             when adding infinities with different signs
 */
 ...
 *             if the result is inexact but the rounding mode is {@code UNNECESSARY}.
 * @throws ArithmeticException
 *             when subtracting infinities with same sign.
 */
 ...
 *
 * Edge cases: - If one of the operands is infinite, an extrapolated point is returned. The points are extrapolated by
 * multiplying with 2. - If operands are infinities with oposite signs, zero is returned. - If operands are equal or
 * {@code this} > {@code b} an exception is thrown.
 *
 * @param b
 ...
 */
protected def signToInfty(s:Int):T={if(s>0)posInf else if(s<0)negInf else throw new ArithmeticException("signToInfinity called with zero.")}}
