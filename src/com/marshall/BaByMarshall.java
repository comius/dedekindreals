...
public class BaByMarshall {
	...
	public static void main(String args[]) {
		...
		// Division by cuts
		MyReal div = new Cut(BigDecimal.ZERO, TWO, x -> new LessThan(new Times(x, new Const(new BigDecimal(7))), new Const(new BigDecimal(1))),
		...
		MyReal two2sqrt2 = new Cut(BigDecimal.ZERO, new BigDecimal(3), x -> new LessThan(new Times(x, new Const(new BigDecimal(7))), new Const(new BigDecimal(1))),
				x -> new LessThan(new Const(new BigDecimal(1)), new Times(x, new Const(new BigDecimal(7)))));
		refineReal(two2sqrt2, 100);
		System.out.println(two2sqrt2);		
		
		
		// Is it a problem for a cut to contain its refinement interval and not clone \sigma^R functions? 
		// Is it ok to reuse cut?
		/*MyReal d4 = new Cut(BigDecimal.ZERO, TWO, 
				x -> new LessThan(
						new Cut(BigDecimal.ZERO, TWO, y -> new LessThan(new Times(x, y), RTWO),
													  y -> new LessThan(RTWO, new Times(x, y))), RTWO),
				x -> new LessThan(RTWO, 
						new Cut(BigDecimal.ZERO, TWO, y -> new LessThan(new Times(x, y), RTWO),
													  y -> new LessThan(RTWO, new Times(x, y)))));
		refineReal(d4, 100);
